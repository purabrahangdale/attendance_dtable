...
const AdminPanel = () => {
  ...
  const fetchTeamHistory = useCallback(async () => {
    try {
      const response = await axios.get('https://attendance-dtable-h8bbsnq71-purabrahangdales-projects.vercel.app/attendance/team-history');
  ...
  const askAI = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('https://attendance-dtable-h8bbsnq71-purabrahangdales-projects.vercel.app/ai/chat', { query });
  ...
  const markInvalid = async (userId: string, date: string) => {
    ...
    try {
      await axios.post('https://attendance-dtable-h8bbsnq71-purabrahangdales-projects.vercel.app/attendance/mark-invalid', { user_id: userId, date });
      fetchTeamHistory();
    } catch (err) {
