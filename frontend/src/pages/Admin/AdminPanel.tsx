...
const AdminPanel = () => {
  ...
  const fetchTeamHistory = useCallback(async () => {
    try {
      const response = await axios.get('http://attendancedtable-production.up.railway.app/attendance/team-history');
  ...
  const askAI = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://attendancedtable-production.up.railway.app/ai/chat', { query });
  ...
  const markInvalid = async (userId: string, date: string) => {
    ...
    try {
      await axios.post('http://attendancedtable-production.up.railway.app/attendance/mark-invalid', { user_id: userId, date });
      fetchTeamHistory();
    } catch (err) {
